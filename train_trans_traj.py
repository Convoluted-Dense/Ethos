...
Transformer-based training pipeline with:
  ...
  - 11 outputs: [x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, scaled_speed]
...
class SteeringDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # BeamNG standard mathematical rotation fix
        adjusted_yaw = -yaw
        ...
        for lf in lookahead_frames:
            fut_idx = min(telemetry_idx + lf, len(self.telemetry) - 1)
            fut_row = self.telemetry[fut_idx]
            
            f_x = _get(fut_row, 'posX', c_x)
            f_y = _get(fut_row, 'posY', c_y)
            ...
            traj_points.extend([loc_x, loc_y])
            
        # Target: [x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, scaled_speed]
        target_list = traj_points + [scaled_speed]
        labels_tensor = torch.tensor(target_list, dtype=torch.float32)
...
class SteeringModelTrans(nn.Module):
    ...
    def __init__(self):
        ...
        # -- Output head: 11 regression values
        self.head = nn.Sequential(
            nn.Dropout(p=0.3),
            nn.Linear(768 + 1, 256),  # +1 for condition
            nn.SiLU(),
            nn.Dropout(p=0.2),
            nn.Linear(256, 11)   # [x1..y5, scaled_speed]
        )
...
# ---------------------------------------------------------------------------
def main():
    wandb.init(project="beamng-ai", name="steering-trans-traj")
    ...
    # -- Loss: Trajectory (Huber) + Speed
    traj_weight  = 0.80
    speed_weight = 0.20
    _huber = nn.SmoothL1Loss(reduction='mean')

    def criterion(outputs, labels):
        # outputs shape: (B, 11), labels shape: (B, 11)
        traj_pred = outputs[:, :10]
        traj_targ = labels[:, :10]
        speed_pred = outputs[:, 10]
        speed_targ = labels[:, 10]
        
        traj_loss = _huber(traj_pred, traj_targ)
        speed_loss = _huber(speed_pred, speed_targ)
        
        total_loss = traj_weight * traj_loss + speed_weight * speed_loss
        return total_loss, traj_loss, speed_loss
    ...
    for epoch in range(epochs):
        ...
        while active_iters:
            ...
            if scaler is not None:
                with torch.amp.autocast('cuda'):
                    ...
                    loss, tr_l, sp_l = criterion(outputs, labels)
            ...
            t_traj += tr_l.item() * n
            t_spd  += sp_l.item() * n
            pbar.set_postfix({'loss': f"{loss.item():.3f}", 'tr': f"{tr_l.item():.3f}", 'dom': domain_name})
            pbar.update(1)
            
        pbar.close()

        N = len(real_train_dataset) + len(diffused_train_dataset)
        t_loss /= N; t_traj /= N; t_spd /= N

        # -- Validation
        model.eval()
        v_loss = v_traj = v_spd = 0.0
        ...
        Nv     = len(val_dataset)
        v_loss /= Nv; v_traj /= Nv; v_spd /= Nv

        current_lr = optimizer.param_groups[0]['lr']
        print(f"Epoch {epoch+1}/{epochs} | "
              f"Train={t_loss:.4f} (tr={t_traj:.4f} sp={t_spd:.4f}) | "
              f"Val={v_loss:.4f} (tr={v_traj:.4f} sp={v_spd:.4f}) | "
              f"LR={current_lr:.2e}")

        wandb.log({
            "Train Loss": t_loss, "Train Traj Loss": t_traj,
            "Train Speed Loss": t_spd,
            "Val Loss": v_loss, "Val Traj Loss": v_traj,
            "Val Speed Loss": v_spd,
        ...
        if v_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_steering_trans_traj_model.pth')
            meta = {
                "max_speed": base_train_dataset.max_speed,
                "version": "trans_vit_traj",
                "seq_len": 4,
                "outputs": ["x1", "y1", "x2", "y2", "x3", "y3", "x4", "y4", "x5", "y5", "scaled_speed"],
                "inputs": ["images", "condition"]
            }
            with open('best_steering_trans_traj_model_meta.json', 'w') as mf:
                json.dump(meta, mf, indent=2)
            print(f"--> Saved best Traj model  Val Loss={v_loss:.4f}  max_speed={base_train_dataset.max_speed:.4f}")
