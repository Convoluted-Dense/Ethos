...
def main():
    ...
    version = input("Choose model version (v2/v3): ").strip().lower()
    if version not in ['v2', 'v3']:
        print("Invalid version, defaulting to v3")
        version = 'v3'
        
    model_path = opt.model if opt.model else f"best_steering_{version}_model.pth"
    print(f"[init] Loading {version.upper()} model from: {model_path}")
    if version == 'v2':
    ...
    # 5-frame buffer for V2 sequence context, or 2-frame for V3
    ...
    try:
        while True:
            ...
            prev_tensor = frame_buffer[-1] if frame_buffer else curr_tensor
            ...
            seq_5 = torch.stack(frames, dim=0).unsqueeze(0)
            ...
            if act_tracker is not None:
                ...
                for stage_name in stages_to_show:
                    ...
                    if feat_full is not None:
                        # Slice the last frame of the sequence for V2 activations, or just take the whole thing for V3
                        feat = feat_full[-1:] if version == 'v2' else feat_full
