...
# ---------------------------------------------------------------------------
def make_parser():
    p = argparse.ArgumentParser(description="Offline Video Prediction using ViT")
    p.add_argument("--video",     default="woxsen.mp4",
                   help="path to input video file")
    p.add_argument("--model",     default="best_steering_trans_model.pth",
...
def main():
    ...
    if not cap.isOpened():
        print(f"ERROR: Could not open {opt.video}")
        sys.exit(1)

    cv2.namedWindow("ViT Video Prediction", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ViT Video Prediction", opt.width, opt.height)
    ...
    def on_trackbar(val):
        nonlocal seek_frame, ignore_trackbar
        if not ignore_trackbar:
            seek_frame = val

    cv2.createTrackbar("Frame", "ViT Video Prediction", 0, max(1, total_frames - 1), on_trackbar)

    # 4-frame buffer for temporal context
    ...
    try:
        while True:
            ...
            cv2.setTrackbarPos("Frame", "ViT Video Prediction", current_frame)
            ignore_trackbar = False
            ...
            vis = draw_hud(raw, pred_steering, pred_speed_kmh, pred_offset, fps_display, False, 
                           0.0, 0.0, current_condition,
            ...
            cv2.imshow("ViT Video Prediction", vis)
