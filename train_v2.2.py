...
# ---------------------------------------------------------------------------
def main():
    wandb.init(project="beamng-ai", name="steering-v2.2-10frame-3output")
    ...
    batch_size   = 16   # Slightly reduced for 6-channel larger feature maps
    epochs       = 25
    lr           = 0.001
    weight_decay = 1e-4
    ...
    for epoch in range(epochs):
        ...
        if v_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_steering_v2.2_model.pth')
            meta = {
            ...
            with open('best_steering_v2.2_model_meta.json', 'w') as mf:
                json.dump(meta, mf, indent=2)
            print(f"--> Saved best V2.2 model  Val Loss={v_loss:.4f}  max_speed={base_train_dataset.max_speed:.4f}")
