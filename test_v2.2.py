...
test_v2.2.py
=============
Real-time inference script for the V2.2 model (EfficientNet + LSTM).
...
# ---------------------------------------------------------------------------
def make_parser():
    p = argparse.ArgumentParser(description="BeamNG CNN real-time inference (V2.2) script")
    p.add_argument("--model",     default="best_steering_v2.2_model.pth",
...
def main():
    ...
    print(f"[init] Loading V2.2 model from: {opt.model}")
    ...
    if not opt.headless:
        cv2.namedWindow("BeamNG CNN V2.2", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("BeamNG CNN V2.2", opt.width, opt.height)
    ...
    try:
        while True:
            ...
            # Build (1, 10, 3, 240, 240) input tensor
            ...
            if not opt.headless:
                ...
                cv2.imshow("BeamNG CNN V2.2", vis)
